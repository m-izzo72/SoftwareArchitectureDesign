package org.softwarearchitecturedesigngroup10.model.commands.selection.groups;// package org.softwarearchitecturedesigngroup10.model.commands;
...
public class UngroupShapesCommand implements Command {
    ...
    private GroupedShapesData groupThatWasUngrouped; // Il gruppo prima di essere separato
    ...
    public UngroupShapesCommand(CanvasModel receiver) {
        ...
        for (Map.Entry<String, ShapeData> entry : receiver.getSelectedShapes().entrySet()) {
            if (entry.getValue() instanceof GroupedShapesData) {
                ...
                this.groupThatWasUngrouped = (GroupedShapesData) entry.getValue().clone(); // Salva un clone del gruppo
